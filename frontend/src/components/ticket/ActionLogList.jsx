...
const getActionType = (actionName) => {
  ...
  if (name.includes('update') || name.includes('edit') || name.includes('modify')) return 'update';
      ...
      <div className={styles.card}>
        <div className={styles.header}>
          <div className={styles.actionInfo}>
            <div className={styles.actionName}>
              ...
              {log.action?.name || 'Unknown Action'}
            </div>
        ...
        <div className={styles.meta}>
          <div className={styles.resolver}>
            <span>👤</span>
            <div className={styles.resolverBadge}>
              {log.user}
            </div>
          </div>
  ...
  if (error) {
    return (
      <div className={styles.error}>
        ❌ Failed to load logs: {typeof error === 'string' ? error : JSON.stringify(error)}
      </div>
    );
  }
  ...
  if (logs.length === 0) {
    return (
      <div className={styles.empty}>
        <div className={styles.emptyText}>No action logs found for this task.</div>
      </div>
    );
  }
