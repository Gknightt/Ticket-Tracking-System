...
export default function WorkflowTab({ timeFilter, analyticsData = {}, loading, error }) {
  ...
  const stepAvgTime = stepPerformance?.map(s => Math.round(s.avg_time_hours || 0)) || [];
    ...
    <div className={styles.chartsGrid}>
      <div className={styles.chartSection}>
        ...
        <div className={styles.chartRow}>
          ...
          {/* Chart 3: Step Performance */}
          <ChartContainer title="Average Step Completion Time">
            <BarChart
              labels={stepLabels}
              dataPoints={stepAvgTime}
              chartTitle="Average Step Completion Time (mins)"
              chartLabel="Minutes"
            />
          </ChartContainer>
        </div>
